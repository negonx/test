...
function delete_tudo($tb) {
    ...
    if ($stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (isset($sql_delete_streams)) {
            $stmt_streams = $pdo->prepare($sql_delete_streams);
            $stmt_streams->execute();
        }
        if (isset($sql_delete_series)) {
            $stmt_series = $pdo->prepare($sql_delete_series);
            $stmt_series->execute();
        }
        ...
        foreach ($tabelas as $tabela) {
            ...
            if ($total == 0) {
                ...
                $stmt_reset_ai = $pdo->prepare($sql_reset_ai);
...
function confirme_delete_categorias($id, $name)
{
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($pdo->exec($sql_delete)) {
            ...
            if ($pdo->exec($sql_delete)) {
                if ($type === 'series') {
                    $pdo->exec($sql_delete_episodios);
                    $pdo->exec($series_seasons);
                }
