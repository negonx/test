...
function mover_clientes_plano($id)
{
    ...
    if ($row = $stmt->fetch()) {
            ...
            $plano_id = $pdo->query("SELECT * FROM planos WHERE admin_id = '$admin_id'");
...
function confirme_mover_clientes_plano($plano_atual, $proximo_plano)
{
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $stmt = $pdo->prepare("UPDATE clientes SET plano = :proximo_plano WHERE plano = :plano_atual AND admin_id = :admin_id");
