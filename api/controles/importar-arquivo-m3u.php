<?php
// PDO migration compatibility
set_time_limit(0);
require_once __DIR__ . '/../../config/bootstrap.php';
require_once(__DIR__.'/../../auth/session.php');

function executarConsulta($pdo, $query, $parametros) {
...
function adicionarCategoria($nome, $type) {
    ...
    $categoria = executarConsulta($pdo, $query, ['nome' => $nome, 'type' => $type])->fetch(PDO::FETCH_ASSOC);
    if ($categoria) return $categoria['id'];

    $query = "INSERT INTO categoria (nome, type, admin_id) VALUES (:nome, :type, :admin_id)";
    executarConsulta($pdo, $query, ['nome' => $nome, 'type' => $type, 'admin_id' => $_SESSION['admin_id']]);
...
function adicionarSeries($nomeSerie, $category_id, $tvgLogo) {
    ...
    $series = executarConsulta($pdo, $query, ['name' => $nomeSerie, 'category_id' => $category_id])->fetch(PDO::FETCH_ASSOC);
    if ($series) return $series['id'];

    $query = "INSERT INTO series (name, category_id, cover) VALUES (:name, :category_id, :cover)";
    executarConsulta($pdo, $query, ['name' => $nomeSerie, 'category_id' => $category_id, 'cover' => $tvgLogo]);
    return $pdo->lastInsertId();
}
...
function adicionarSeasonsSeries($temporada, $episode_count, $series_id) {
    ...
    $temporadaExistente = executarConsulta($pdo, $query, ['series_id' => $series_id, 'season_number' => $temporada])->fetch(PDO::FETCH_ASSOC);
    ...
    if ($temporadaExistente) {
        ...
        executarConsulta($pdo, $query, ['episode_count' => $episode_count, 'series_id' => $series_id, 'season_number' => $temporada]);
...
function verificarLinksExistentes($pdo, $urls) {
...
function adicionarLinks($links) {
    ...
    try {
        ...
        $linksExistentes_episodios = verificarLinksExistentes($pdo, $urls);
        foreach ($links as $link) {
            ...
            ((preg_match('/\/movie\b/i', $url)) ? 'movie' : null));
                ...
                if (!isset($_SESSION['cache'][$groupTitle]) && $groupTitle != null) {
                    ...
                    $categoria = executarConsulta($pdo, $queryCategoria, ['nome' => $groupTitle, 'type' => $type])->fetch(PDO::FETCH_ASSOC);

                    if ($categoria) {
                        $category_id = $categoria['id'];
                    } else {
                        $queryInsertCategoria = "INSERT INTO categoria (nome, type, admin_id) VALUES (:nome, :type, :admin_id)";
                        executarConsulta($pdo, $queryInsertCategoria, [
                            'nome' => $groupTitle,
                ...
                if ($type === 'series' && $url != null) {
                    ...
                    if (!isset($_SESSION['cache'][$groupTitle]['series'][$nomeSerie])) {
                        ...
                        $serieExistente = executarConsulta($pdo, $querySeries, ['name' => $nomeSerie, 'category_id' => $category_id])->fetch(PDO::FETCH_ASSOC);
                        ...
                        if ($serieExistente) {
                            ...
                            executarConsulta($pdo, $queryInsertSeries, [
                                'name' => $nomeSerie,
                                'category_id' => $category_id,
                                'cover' => $tvgLogo
                            ]);
                    ...
                    if (!in_array($temporada, $_SESSION['cache'][$groupTitle]['series'][$nomeSerie]['seasons'])) {
                        ...
                        if ($temporadaExistente) {
                            ...
                            $query = "UPDATE series_seasons SET episode_count = :episode_count WHERE series_id = :series_id AND season_number = :season_number";
                            executarConsulta($pdo, $query, ['episode_count' => $episodioNum, 'series_id' => $series_id, 'season_number' => $temporada]);
                ...
                if (($type === 'live' || $type === 'movie') && $url != null) {
                    ...
                    $execute = executarConsulta($pdo, $query, [
                        'link' => $url, 
                        'category_id' => $category_id, 
                        'name' => $channelName,
                        'stream_type' => $type, 
