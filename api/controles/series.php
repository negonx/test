...
function editar_series($id)
{
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $categoria = $pdo->query("SELECT * FROM categoria WHERE admin_id = '$admin_id' and type = 'series' ORDER BY nome ASC");
        $categoriaOptions = "";
        $categoriaOptions2 = "";
        ...
        $temporadas = $pdo->query("SELECT * FROM series_seasons WHERE series_id = '$id' ORDER BY season_number ASC");
        $temporadasOptions = "";
...
function confirme_delete_series($id, $name)
{
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($stmt_delete->execute()) {
            ...
            $stmt_episodes = $pdo->prepare($sql_delete_episodes);
            ...
            $stmt_seasons = $pdo->prepare($sql_delete_seasons);
...
function ver_episodios($series_id, $temporada, $episodio)
{
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $episode = $pdo->query("SELECT * FROM series_episodes WHERE series_id = '$series_id' AND season ='$temporada'  ORDER BY episode_num ASC");
        $episodeOptions = "";
...
function confirme_adicionar_episodios($series_id, $temporada) {
    ...
    if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        if ($row['admin'] == 1) {
            ...
            $stmt_episodio = $pdo->prepare($sql_episodios);
