...
// Compatibilidade 8.1: aceita senha legada e hash seguro
if($result && isset($result['senha']) && strpos($result['senha'], '$2y$')===0){
    if(!password_verify($password,$result['senha'])) { $result=false; }
}
if ($result && isset($result['Vencimento']) && strtotime($result['Vencimento']) < time()) { $result = false; }
if (!$result) {
    http_response_code(401); // 401 Unauthorized
...
if (isset($_GET['action']) && isset($_GET['vod_id']) && $_GET['action'] === 'get_vod_info') {
    ...
    $query = $pdo->prepare("SELECT * FROM streams WHERE id = :vod_id");
...
if (isset($_GET['action'], $_GET['series_id']) && $_GET['action'] === 'get_series_info') {
    ...
    $query = $pdo->prepare("SELECT * FROM series WHERE id = :series_id");
    ...
    $query = $pdo->prepare("SELECT * FROM series_seasons WHERE series_id = :series_id");
    $query->execute([":series_id" => $series_id]);
    $seasons = [];
    ...
    $query = $pdo->prepare("SELECT * FROM series_episodes WHERE series_id = :series_id ORDER BY season, episode_num");
    $query->execute([":series_id" => $series_id]);
    $episodes = [];
